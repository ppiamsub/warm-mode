...
- **Viewer ออกจากบัญชีเองได้:** ปุ่ม "ออกจากบัญชีนี้" ในหน้า Viewer · `POST /api/me/leave` ลบ membership viewer + ปลดผูก person + reset session เป็นฐาน → เด้งไป `/accounts`
- **Fix: admin ที่เข้าร่วมบัญชีตัวเองเป็น viewer แล้วกลับโหมด admin ไม่ได้:** `memberships` unique ต่อ (user,book) → join เป็น viewer เดิม overwrite สิทธิ์ admin · แก้ `/api/join`: ถ้าเป็น admin ของบัญชีนั้นอยู่แล้ว **ไม่ลดสิทธิ์** (คงโหมด admin, ไม่ผูก person) · คนที่ค้างอยู่แล้วกู้คืนได้โดยกรอก **Book Code** ซ้ำ (upsert เป็น admin)
...
- **แบรนด์: โลโก้ไอคอนเป็นลายน้ำ + โลโก้หน้าแรกวงกลม:** `GreenHeader` เปลี่ยนลายน้ำมาใช้ `src/logo_icon.png` (พื้นหลังโปร่งใส blend กับเขียวเนียนกว่า) · `BrandLogo` ตัด `src/logo.png` เป็น **วงกลม** (`overflow:hidden`) **ไม่มีสีพื้นหลัง** เหลือแค่ **เงา + แสงเขียวเรือง** (boxShadow) · ซูมโฟกัสตัวมาสคอต (`scale(1.32)`) ดันคำว่า "WARM MODE" ที่ฝังในรูปให้พ้นวงกลม
- **Fix (สำคัญ): Personal Code ที่ระบบ gen ใช้เข้าร่วมไม่ได้:** `generatePersonalCode` เดิมเอา `name.charAt(0)` มาเป็น prefix (ตัวเดียว เช่น `M-8267`) และ fallback `randomBlock(2)` ดึงจาก alphabet ที่**มีตัวเลข** (เช่น `3X-5441`) → ไม่ผ่าน `isValidCodeFormat` (`[A-Z]{2}`) → **viewer กรอกโค้ดเข้าร่วมไม่ได้เลย** (ชื่อไทยส่วนใหญ่รอดเพราะ fallback สุ่มได้ 2 ตัวอักษรพอดี จึงแอบซ่อนบัค) · แก้: prefix เป็น **ตัวอักษร A–Z 2 ตัวเสมอ** (ดึงจากชื่อได้เท่าไรเติมสุ่มให้ครบ) + **ผ่อน `CODE_RE` เป็น `[A-Z0-9]{1,2}-?\d{4,6}`** ให้โค้ดเก่าที่ gen ผิดรูปในฐานข้อมูลยังใช้ได้ · verify: โค้ดชื่อ Latin/ไทย join สำเร็จทั้งหมด + โค้ดเก่า `M-8267`/`3X-5441` join ได้แล้ว
- **Viewer กดออกจากบัญชีได้จากหน้า Hub:** `/api/me/leave` รับ `bookId` ทาง body ได้ (ดึง `person_id` จาก membership เอง ไม่พึ่ง session) → ออกจากบัญชี viewer ใด ๆ ได้โดยตรงจาก Hub (ไม่ต้องเข้าไปหน้า viewer แล้วเลื่อนหาปุ่มล่างสุด) · การ์ดบัญชีสมาชิกใน `/accounts` เพิ่มปุ่ม **"ออกจากบัญชี"** (แยกปุ่มเลือก/ปุ่มออก, กัน nested button) · ปุ่มเดิมในหน้า `/viewer` (ยิงแบบไม่มี body) ยังใช้ได้ (fallback เป็น `session.bookId`)
- **ตรวจสอบบัค "สลับกลับเป็น admin ไม่ได้":** reproduce end-to-end แล้ว (admin ของบัญชี A + join บัญชี B เป็น viewer → เลือก A กลับ) — **ทำงานถูกต้อง** (`/api/accounts/select` คืน role=admin, `/api/overview` 200) เพราะ fix 176aa2a คุ้มครองไว้แล้ว · ถ้ายังเจออาการนี้ น่าจะเป็น **ข้อมูลเก่าค้าง** (membership ถูก overwrite เป็น viewer ก่อนมี fix) → กู้คืนโดยกรอก **Book Code** ของบัญชีนั้นซ้ำ (upsert กลับเป็น admin)
