...
เอกสารฉบับนี้กำหนดบทบาท หน้าที่ และข้อจำกัดสำหรับ AI Agents ที่จะเข้ามาช่วยพัฒนาโค้ดในโปรเจกต์ Debt Ledger ซึ่งเป็นระบบติดตามยอดเงินที่ต้องเก็บจากลูกหนี้
...
* **งานที่ต้องทำ:** * สร้าง Endpoint `/api/auth/line` เพื่อรับค่าและตรวจสอบ `line_uid` ในฐานข้อมูล
  ...
  * พัฒนา API สำหรับ CRUD ข้อมูลบุคคล (People) และรายการหนี้ (Entries)
...
* **งานที่ต้องทำ:** * สร้างและปรับปรุง Schema ของตาราง `books`, `people` และ `entries`
  * จัดการ Constraint ของฐานข้อมูล เช่น การตั้งค่า `ON DELETE CASCADE` เพื่อลบรายการหนี้ทั้งหมดหากรายชื่อลูกหนี้ถูกลบ
...
* **กฎเหล็ก 3:** ห้ามแก้ไขโครงสร้างที่กระทบต่อ `line_uid` ในตาราง `books` และ `people` เนื่องจากเป็นหัวใจสำคัญของการทำ Auto-login
